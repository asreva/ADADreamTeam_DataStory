...
Demythify somehow the rumors that those who have a lot of friends do not have good friends.
...
The results of the regressions computed are not representative as the R-squared score is 0.307. Maybe if we had the happiness score of each user we could use the DataFrame "users_all_std" to see if there is any relation between frienship quality and quantity and happiness. We hypothesis that there is and that with a larger and more precise data it could be proved.
